...
def naive_dqn_loss_V2(batch, main_net, target_net, gamma, batch_size = 32, device = "cuda", optimizer = 'opt'):
    ...
    #print(predicted_state_action_values)
    #print(predicted_state_action_values.size())
    # create target, (Ground truth ) with next state values, discounted by Gamma 
    #  dims = batch_size,number_of_actions
   ...
   # print(next_state_values)
    ...
    # inplace transformation, discount by Gamma 
    next_state_values = next_state_values * gamma
    
    #print(next_state_values)
    #next_state_values = next_state_values.cpu()
    #next_state_values_np = next_state_values.numpy()
    #print('=======================================================')
    ...
    for prediction, reward, action,  tensor in zip(predicted_state_action_values,tensor_rewards,tensor_actions, next_state_values):
        ...
        # step in here to see what is inside
        #my_loss  = ((prediction - target)**2).mean()
        #print(my_loss)
        #print(sample_loss)
        #print(type(sample_loss))
